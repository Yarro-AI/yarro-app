...
import { SimulateTicketCard } from '@/components/onboarding/simulate-ticket-card'
...
export default function DashboardPage() {
  ...
  const simCardAnchorRef = useRef<HTMLDivElement>(null)
  const supabase = createClient()

  // Rent entries are auto-generated by DB triggers (on tenant assign / rent config)
  // and the monthly cron (generate-monthly-rent). No fire-and-forget needed.
      ...
      {propertyManager && step === 'tour' && (
        <DashboardTour
          ...
          onTourComplete={() => {
            ...
            // SimulateTicketCard will appear inline in Needs Action.
          }}
        />
      )}

      {/* Simulation modal — triggered by clicking SimulateTicketCard */}
      {simulationActive && propertyManager && (
        <SimulationOverlay
          pmId={propertyManager.id}
          onComplete={async () => {
            await refreshPM()
            ...
            <div className="flex flex-col flex-1 min-h-0 overflow-y-auto">
              {actionable.length === 0 && onboardingChecklist.length === 0 ? (
                ...
                <>
                  ...
                  {/* Anchor for SimulateTicketCard position measurement — actual card rendered fixed above overlay */}
                  {step === 'simulate' && (
                    <div ref={simCardAnchorRef} className="h-[76px] mx-4 mt-4" />
                  )}
                  {/* Hide ticket list during simulate — SimulateTicketCard is the only focus */}
                  {step !== 'simulate' && (
                    <JobsList
...
/**
 * Renders SimulateTicketCard as a fixed-position element above the dim overlay.
 * Measures position from an anchor ref inside the Needs Action column so it
 * visually appears in the right spot but lives outside the stacking context.
 */
function FixedSimulateCard({ anchorRef, onClick }: { anchorRef: React.RefObject<HTMLDivElement | null>; onClick: () => void }) {
  ...
  return (
    ...
    >
      <SimulateTicketCard onClick={onClick} />
    </div>
  )
}
