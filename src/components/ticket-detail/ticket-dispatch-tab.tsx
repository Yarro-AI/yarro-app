...
const STATUS_STYLES: Record<string, string> = {
  ...
  declined_by_manager: 'bg-red-500/10 text-red-700',
  no_response: 'bg-red-500/10 text-red-700',
  send_failed: 'bg-red-500/10 text-red-700',
    ...
    for (let di = 0; di < dispatchRounds.length; di++) {
      ...
      if (roundTimedOut) {
        ...
        status = rawStatus === 'approved' ? 'approved'
          : rawStatus === 'declined_by_manager' ? 'declined_by_manager'
          : rawStatus === 'no_response' ? 'no_response'
          : rawStatus === 'send_failed' ? 'send_failed'
          : rawStatus === 'replied' ? 'quoted'
          : rawStatus === 'sent' ? 'sent'
          : 'pending'
      } else {
        status = 'sent'
