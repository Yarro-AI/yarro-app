-- ============================================================
-- PROTECTED RPC CHANGE: c1_get_dashboard_todo — compliance-aware labels
-- ============================================================
-- Safe Modification Protocol:
--   Backup: supabase/rollbacks/rollback_dashboard_compliance_labels.sql
--   Approved by: Adam (compliance dashboard flow — match maintenance state progression)
--
-- Changes:
--   1. LEFT JOIN c1_compliance_certificates for cert expiry_date
--   2. Compliance-aware issue_summary: "EICR renewal — expired 159d ago"
--   3. compliance_pending action branches: "Dispatch contractor" + cert context
--   4. Stale compliance_pending (>48h): priority_bucket → HIGH, is_past_timeout → true
--      (stays in Needs Action, not Stuck — PM must act, not external blocker)
--   5. priority_score +30 for compliance_pending (up from default +5)
-- ============================================================
    ...
    SELECT
      ...
      CASE
        WHEN t.category = 'compliance_renewal' AND cc.expiry_date IS NOT NULL AND cc.expiry_date < CURRENT_DATE
          THEN COALESCE(t.issue_title, 'Compliance') || ' — expired ' || (CURRENT_DATE - cc.expiry_date) || 'd ago'
        WHEN t.category = 'compliance_renewal' AND cc.expiry_date IS NOT NULL
          THEN COALESCE(t.issue_title, 'Compliance') || ' — expires in ' || (cc.expiry_date - CURRENT_DATE) || 'd'
