-- ============================================================
-- PROTECTED RPC CHANGE: c1_get_dashboard_todo — human-readable labels + cron registration
-- ============================================================
-- Safe Modification Protocol:
--   Backup: supabase/rollbacks/rollback_dashboard_label_days.sql
--   Approved by: Adam (compliance labels + cron + page state consistency)
--
-- Changes:
--   1. All "Xd ago" labels → "X days ago" for human-readable text
--   2. Register contractor + landlord timeout cron jobs (if not already registered)
-- ============================================================
...
BEGIN
  ...
  scored AS (
    SELECT
      ...
      -- Compliance-aware issue_summary: show cert type + expiry context
      CASE
        WHEN t.category = 'compliance_renewal' AND cc.expiry_date IS NOT NULL AND cc.expiry_date < CURRENT_DATE
          THEN COALESCE(t.issue_title, 'Compliance') || ' — expired ' || (CURRENT_DATE - cc.expiry_date) || ' days ago'
        WHEN t.category = 'compliance_renewal' AND cc.expiry_date IS NOT NULL
          THEN COALESCE(t.issue_title, 'Compliance') || ' — expires in ' || (cc.expiry_date - CURRENT_DATE) || ' days'
        ELSE COALESCE(t.issue_title, LEFT(t.issue_description, 100))
      ...
      t.ooh_outcome_at,
      -- Cert expiry for compliance context
      cc.expiry_date AS cert_expiry_date,
      -- action_type
      CASE
        ...
        -- compliance_pending stays FOLLOW_UP (Needs Action) — PM must act, not external blocker
        WHEN t.next_action_reason = 'compliance_pending' THEN 'FOLLOW_UP'
        ELSE 'FOLLOW_UP'
      END AS action_type,
      -- action_label
      CASE
        ...
        -- Compliance pending: dispatch contractor
        WHEN t.next_action_reason = 'compliance_pending' THEN 'Dispatch contractor'
        -- Existing needs attention states
      ...
      CASE
        ...
        WHEN t.next_action_reason = 'awaiting_booking'
          ...
          THEN 'Contractor accepted ' ||
            ...
            ' days ago but hasn''t booked — chase booking'
        WHEN t.next_action_reason = 'scheduled'
          AND t.scheduled_date IS NOT NULL
          AND t.scheduled_date::date < CURRENT_DATE
        ...
        -- Compliance pending: cert-aware context
        WHEN t.next_action_reason = 'compliance_pending' AND cc.expiry_date IS NOT NULL AND cc.expiry_date < CURRENT_DATE
          AND EXTRACT(EPOCH FROM (now() - COALESCE(m.updated_at, t.date_logged))) / 3600 > 48
          THEN COALESCE(t.issue_title, 'Certificate') || ' expired ' || (CURRENT_DATE - cc.expiry_date) ||
            ' days ago — dispatch a contractor. Ticket ' ||
            ROUND(EXTRACT(EPOCH FROM (now() - COALESCE(m.updated_at, t.date_logged))) / 3600)::text ||
            'h old with no contractor'
        WHEN t.next_action_reason = 'compliance_pending' AND cc.expiry_date IS NOT NULL AND cc.expiry_date < CURRENT_DATE
          THEN COALESCE(t.issue_title, 'Certificate') || ' expired ' || (CURRENT_DATE - cc.expiry_date) ||
            ' days ago — dispatch a contractor for renewal'
        WHEN t.next_action_reason = 'compliance_pending' AND cc.expiry_date IS NOT NULL AND cc.expiry_date >= CURRENT_DATE
          THEN COALESCE(t.issue_title, 'Certificate') || ' expires in ' || (cc.expiry_date - CURRENT_DATE) ||
            ' days — dispatch a contractor for renewal'
        WHEN t.next_action_reason = 'compliance_pending'
          THEN 'Compliance renewal ticket — dispatch a contractor'
      ...
      (
        ...
        + CASE
          ...
          -- compliance_pending: boosted to surface in Needs Action
          WHEN t.next_action_reason = 'compliance_pending' THEN 30
      ...
      CASE
        ...
        WHEN t.next_action_reason = 'scheduled'
          ...
          AND t.scheduled_date::date < CURRENT_DATE
          THEN 'HIGH'
        -- Dedicated clock escalations → HIGH
        WHEN t.next_action_reason = 'allocated_to_landlord'
          AND t.landlord_allocated_at IS NOT NULL
          AND EXTRACT(EPOCH FROM (now() - t.landlord_allocated_at)) / 3600 > 72
          THEN 'HIGH'
        ...
        WHEN t.next_action_reason = 'ooh_in_progress'
          ...
          AND EXTRACT(EPOCH FROM (now() - t.ooh_outcome_at)) / 3600 > 48
          THEN 'HIGH'
        -- Stale compliance_pending (>48h no contractor) → HIGH (stays in Needs Action, just priority escalation)
        WHEN t.next_action_reason = 'compliance_pending'
          AND EXTRACT(EPOCH FROM (now() - COALESCE(m.updated_at, t.date_logged))) / 3600 > 48
          THEN 'HIGH'
    ...
    -- NEW: cert expiry for compliance-aware labels
  ...
  SELECT jsonb_build_object(
    ...
    'is_past_timeout', CASE
      ...
      WHEN s.next_action_reason = 'ooh_in_progress'
        ...
        THEN true
      -- Stale compliance_pending timeout flag
      WHEN s.next_action_reason = 'compliance_pending'
        AND EXTRACT(EPOCH FROM (now() - s.waiting_since)) / 3600 > 48
        THEN true
      ELSE false
    END
  )
...
-- ============================================================
-- Register contractor + landlord timeout cron jobs
-- These functions exist but were never scheduled via pg_cron.
-- Using IF NOT EXISTS pattern via DO block to avoid duplicate registration.
-- ============================================================

DO $$
BEGIN
  -- Contractor timeout: marks unresponsive contractors, cycles to next
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'contractor-timeout-check') THEN
    PERFORM cron.schedule('contractor-timeout-check', '*/15 * * * *', 'SELECT c1_contractor_timeout_check()');
  END IF;

  -- Landlord timeout: chases landlords who haven't responded
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'landlord-timeout-check') THEN
    PERFORM cron.schedule('landlord-timeout-check', '*/15 * * * *', 'SELECT c1_landlord_timeout_check()');
  END IF;
END $$;

-- One-time: trigger contractor timeout for stuck tickets
SELECT c1_contractor_timeout_check();
