-- ============================================================
-- PROTECTED RPC CHANGE: c1_get_dashboard_todo_extras — action verb labels
-- ============================================================
-- Safe Modification Protocol:
--   Backup: supabase/rollbacks/rollback_extras_action_labels.sql
--   Approved by: Adam (badge/label overhaul)
--
-- Changes:
--   action_label → action verbs matching core verb system
--   Compliance: "Renew certificate" / "Schedule renewal" / "Complete certificate"
--   Rent: "Chase arrears" / "Follow up payment"
--   Tenancy: "Update tenancy" / "Review tenancy"
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'action_type',         CASE
                           ...
                           END,
    'action_label',        CASE
                             WHEN ci.reason_key = 'compliance_incomplete' THEN 'Complete certificate'
                             WHEN ci.reason_key = 'compliance_expired' THEN 'Renew certificate'
                             ELSE 'Schedule renewal'
                           END,
    'action_context',      CASE
                             WHEN ci.reason_key = 'compliance_incomplete'
  ...
  -- Rent: overdue or partial — 4-tier priority system
  SELECT jsonb_build_object(
    ...
    'action_label',        CASE
                             WHEN ri.rent_status = 'partial' THEN 'Follow up payment'
                             ELSE 'Chase arrears'
                           END,
    'action_context',      CASE
  ...
  SELECT jsonb_build_object(
    ...
    'action_label',        CASE WHEN ti.reason_key = 'tenancy_expired' THEN 'Update tenancy' ELSE 'Review tenancy' END,
    'action_context',      CASE
                             WHEN ti.reason_key = 'tenancy_expired'
