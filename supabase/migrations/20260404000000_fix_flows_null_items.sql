-- =============================================================
-- Fix: WhatsApp Flows parsing fails when page items are null
--
-- Bug: When PM skips the markup page in the Flows template,
-- Twilio sends "items": null (JSON null). jsonb_array_elements
-- throws on JSON null, the EXCEPTION block catches it and
-- silently aborts ALL parsing — including the valid decision page.
--
-- Fix: Guard jsonb_array_elements with CASE to skip null/non-array items.
--
-- PROTECTED RPC: c1_inbound_reply (Safe Modification Protocol applied)
-- Backup: Full function copied from 20260327041845_remote_schema.sql lines 4430-4959
-- Change: 4 occurrences of jsonb_array_elements(page->'items') wrapped with null guard
-- =============================================================
...
BEGIN
  ...
  -- FIX: Guard jsonb_array_elements against JSON null items.
  -- Twilio sends "items": null when PM skips a Flows page.
  --------------------------------------------------------------------
  IF p_interactive_data IS NOT NULL AND p_interactive_data <> '' THEN
    BEGIN
