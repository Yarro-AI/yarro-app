-- ============================================================
-- PROTECTED RPC CHANGE: Remove category split logic from RPCs
-- ============================================================
-- Safe Modification Protocol:
--   Backup: supabase/rollbacks/rollback_remove_rpc_split_logic.sql
--   Approved by: Adam (callers provide clean data, RPCs just write)
--
-- Principle: The source of the data provides the right fields.
-- The RPC just writes. No translation, no guessing.
--
-- Changes:
--   c1_create_ticket: reads category + maintenance_trade from _issue JSONB
--     (edge function now sends both). No split logic.
--   c1_create_manual_ticket: new p_maintenance_trade parameter.
--     Callers send route + trade explicitly. No split logic.
-- ============================================================


-- ═══════════════════════════════════════════════════════════════
-- 1. c1_create_ticket — reads both fields from _issue, no split
-- ═══════════════════════════════════════════════════════════════
...
begin
  ...
  v_category := _issue->>'category';                -- "maintenance" (route, set by edge fn)
  v_maintenance_trade := _issue->>'maintenance_trade';  -- "Plumber" (trade, set by edge fn)
  ...
  -- Contractor lookup uses maintenance_trade (the trade value for JSONB key matching)
  if v_property_id is not null and v_maintenance_trade is not null and v_maintenance_trade <> '' then
    ...
    if v_property_id is null or v_maintenance_trade is null or v_maintenance_trade = '' then
      v_should_handoff := true;
    end if;
  end if;
  ...
  values (
    ...
    _issue->>'issue_title',
    v_category,             -- route from caller
    v_maintenance_trade,    -- trade from caller
    _issue->>'priority',
    v_images,
    v_convo.id,
...
-- ═══════════════════════════════════════════════════════════════
-- 2. c1_create_manual_ticket — explicit p_maintenance_trade param, no split
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- No split logic — write what was passed
  INSERT INTO public.c1_tickets (
